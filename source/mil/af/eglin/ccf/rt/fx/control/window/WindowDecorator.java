...
public class WindowDecorator extends VBox
{
    ...
    private final CustomDecorationWindowProcess windowProcEx;
     ...
     */
    public WindowDecorator(Stage stage, String title, Node node)
    {
        this(stage, title, node, true, true, true);
    }
     ...
     */
    public WindowDecorator(Stage stage, String title, Node node, boolean fullScreen, boolean max, boolean min)
    {
        primaryStage = stage;
        this.title.set(title);
        this.primaryStage.setTitle(title);
        windowProcEx = new CustomDecorationWindowProcess();
        ...
        primaryStage.fullScreenProperty().addListener((o, oldVal, newVal) ->
        {
            if (newVal)
            {
                ...
                /*
                 * note the border property MUST NOT be bound to another
                 * property when going full screen mode, thus the binding will
                 * be lost if exisited
                 */
        ...
        ChangeListener<? super Boolean> stageShowingListener = new ChangeListener<Boolean>()
        {
            @Override
            public void changed(ObservableValue<? extends Boolean> ov, Boolean oldVal, Boolean newVal)
            {
                if (newVal.booleanValue() && !isInitialized.getAndSet(true))
                {
        ...
        primaryStage.showingProperty().addListener(stageShowingListener);
...
//        try
//        {
//            TKStage tkStage = this.primaryStage.impl_getPeer();
...
//            hwnd.setPointer(new Pointer((Long) nativeHandle));
//        }
//        catch (Throwable e)
//        {
...
//        return hwnd;
    ...
    private void initializeContainers(Node node, boolean fullScreen, boolean max, boolean min)
    {
        ...
        title.bind(primaryStage.titleProperty()); // binds title to the
                                                  // primaryStage's title
